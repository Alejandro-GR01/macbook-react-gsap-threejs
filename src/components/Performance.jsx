...
const Performance = () => {
    ...
    useGSAP(() => {
        //text animation fade in move up in enter views
        gsap.fromTo(
            ".content p",
            { opacity: 0, y: 10 },
            {
        ...
        <section ref={sectionRef} id="performance">
            ...
            <div className="wrapper">
                {performanceImages.map(({ id, src }, index) => (
                    <picture key={id}>
                        ...
                        <img className={id} src={src} alt={id || `Performance Image #${index + 1}`} loading="lazy" />
                    </picture>
            ...
            <div className="content">
                <p>Run graphics-intensive workflows with a responsiveness that keeps up with your imagination. The M4 family of chips features a GPU with a second-generation hardware-accelerated ray tracing engine that renders images faster, so
                    <span className="text-white">gaming feels more immersive and realistic than ever.</span> And Dynamic Caching optimizes fast on-chip memory to dramatically increase average GPU utilization--driving a huge performance boost for the most demanding pro apps and games.</p>
            </div>
        </section>
    )
}
